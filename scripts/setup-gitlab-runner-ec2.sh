...
# Install GitLab Runner as a service (will start after registration)
echo "Setting up GitLab Runner service..."
if [ -f /etc/systemd/system/gitlab-runner.service ]; then
    echo "GitLab Runner service already exists. Stopping and uninstalling..."
    gitlab-runner stop 2>/dev/null || true
    gitlab-runner uninstall 2>/dev/null || true
    systemctl daemon-reload
fi
gitlab-runner install --user ec2-user
...
echo ""
echo "2. Expand the 'New project runner' section"
echo ""
echo "3. Configure the runner:"
echo "   - Description: simple-time-service-ec2-runner"
echo "   - Tags: project-specific"
echo "   - Run untagged jobs: No (unchecked)"
echo ""
echo "4. Click 'Create runner'"
echo ""
echo "5. Copy the authentication token shown on the page"
echo ""
echo "6. Register the runner on this EC2 instance:"
...
echo "     --description simple-time-service-ec2-runner \\"
echo "     --tag-list project-specific \\"
echo "     --run-untagged=false"
echo ""
echo "7. Configure Docker-in-Docker (required for docker buildx):"
echo "   Edit the runner config:"
echo "   sudo nano /etc/gitlab-runner/config.toml"
echo ""
echo "   Find the [runners.docker] section and add:"
echo "   privileged = true"
echo ""
echo "   Then restart the runner:"
echo "   sudo gitlab-runner restart"
echo ""
echo "   Note: The service is already installed and will start automatically after registration."
echo ""
echo "7. Verify it's running:"
echo "   sudo gitlab-runner status"
echo ""
echo "8. Check runner in GitLab UI:"
echo "   Project → Settings → CI/CD → Runners"
echo "   Should see runner online with tag 'project-specific'"
echo ""
echo "=========================================="
echo "REINSTALLATION (if needed):"
echo "=========================================="
echo ""
echo "To uninstall and reinstall GitLab Runner:"
echo ""
echo "1. Stop and uninstall the service:"
echo "   sudo gitlab-runner stop"
echo "   sudo gitlab-runner uninstall"
echo ""
echo "2. Remove runner configuration (optional, to start fresh):"
echo "   sudo rm -rf /etc/gitlab-runner/config.toml"
echo ""
echo "3. Uninstall the package:"
echo "   sudo dnf remove -y gitlab-runner"
echo ""
echo "4. Re-run this setup script:"
echo "   sudo ./setup-gitlab-runner-ec2.sh"
echo ""
echo "5. Register the runner again (use new token from GitLab UI)"
echo ""
echo "=========================================="
