...
install_efk() {
    ...
    helm upgrade --install kibana elastic/kibana -n logging --set elasticsearchHosts=https://elasticsearch-master.logging.svc.cluster.local:9200 --set extraEnvs[0].name=ELASTICSEARCH_SSL_VERIFICATIONMODE --set extraEnvs[0].value=none --set resources.requests.memory=512Mi --set resources.requests.cpu=500m --wait --timeout 10m &>/dev/null || warn "Kibana may need more time"
